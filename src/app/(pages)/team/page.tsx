...
export default async function TeamGrid() {
  const heroData = await getHero("team"); // Fetch data dynamically
  ...
  return (
    <div>
      <Hero
        isHomePage={false}
        heading={heroData?.hero?.heading || "Our Dynamic Team"}
        subheading={heroData?.hero?.subheading || "Meet our passionate experts"}
        heroImage={heroData?.hero?.heroImage || teamwork}
        altText={heroData?.hero?.altText || "Team Banner"}
      />
          ...
          >
            <div className="flex flex-col items-center pt-6">
              <div className="w-12 h-12 rounded-full bg-blue-50 flex items-center justify-center mb-4">
                {team.icon && <team.icon className="w-6 h-6 text-blue-600" />}
              </div>
              <h3 className="font-semibold text-lg text-center">{team.title}</h3>
            </div>
            <div>
